...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_id'])) {
    ...
    if (!$error) {
        if ($upload_dok) {
            $sql = "UPDATE izin SET jenis_izin=?, no_izin=?, masa_berlaku=?, ket=?, dok=? WHERE id=?";
            $stmt = mysqli_prepare($conn, $sql);
            mysqli_stmt_bind_param($stmt, "sssssi", $jenis_izin, $no_izin, $masa_berlaku, $ket, $upload_dok, $id_post);
        } else {
            $sql = "UPDATE izin SET jenis_izin=?, no_izin=?, masa_berlaku=?, ket=? WHERE id=?";
            $stmt = mysqli_prepare($conn, $sql);
...
$query = "SELECT * FROM izin ORDER BY id ASC";
...
<head>
    <meta charset="UTF-8" />
    <title>Daftar Izin PKS SEI PAGAR</title>
    <style>
        body {
...
.btn-tambah:hover {
    ...
    <div class="header-container">
        <h2>Daftar Izin PKS SEI PAGAR</h2>
        <a href="seipagartambah.php" class="btn-tambah">Tambah</a>
    </div>
