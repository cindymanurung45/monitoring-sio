...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_id'])) {
    ...
    if (!$error) {
        if ($upload_dok) {
            $sql = "UPDATE izin2 SET jenis_izin=?, no_izin=?, masa_berlaku=?, ket=?, dok=? WHERE id=?";
            $stmt = mysqli_prepare($conn, $sql);
            mysqli_stmt_bind_param($stmt, "sssssi", $jenis_izin, $no_izin, $masa_berlaku, $ket, $upload_dok, $id_post);
        } else {
            $sql = "UPDATE izin2 SET jenis_izin=?, no_izin=?, masa_berlaku=?, ket=? WHERE id=?";
            $stmt = mysqli_prepare($conn, $sql);
...
$edit_data = null;
if ($edit_id > 0) {
    $sql = "SELECT * FROM izin2 WHERE id = ?";
    $stmt = mysqli_prepare($conn, $sql);
...
$query = "SELECT * FROM izin2 ORDER BY id ASC";
...
<head>
    <meta charset="UTF-8" />
    <title>PKS TERANTAM - Surat Izin Operasi (SIO)</title>
    <style>
        body {
...
<body>
    <div class="header-container">
        <h2>Daftar Izin PKS TERANTAM</h2>
        <a href="terantamtambah.php" class="btn-tambah">Tambah</a>
    </div>
