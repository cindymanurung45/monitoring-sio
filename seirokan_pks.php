...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_id'])) {
    ...
    if (!$error) {
        if ($upload_dok) {
            $sql = "UPDATE izin12 SET jenis_izin=?, no_izin=?, masa_berlaku=?, ket=?, dok=? WHERE id=?";
            $stmt = mysqli_prepare($conn, $sql);
            mysqli_stmt_bind_param($stmt, "sssssi", $jenis_izin, $no_izin, $masa_berlaku, $ket, $upload_dok, $id_post);
        } else {
            $sql = "UPDATE izin12 SET jenis_izin=?, no_izin=?, masa_berlaku=?, ket=? WHERE id=?";
            $stmt = mysqli_prepare($conn, $sql);
...
$edit_data = null;
if ($edit_id > 0) {
    $sql = "SELECT * FROM izin12 WHERE id = ?";
    $stmt = mysqli_prepare($conn, $sql);
...
$query = "SELECT * FROM izin12 ORDER BY id ASC";
...
<head>
    <meta charset="UTF-8" />
    <title>Daftar Izin PKS SEI ROKAN</title>
    <style>
        body {
...
.btn-tambah:hover {
    ...
    <div class="header-container">
        <h2>Daftar Izin PKS SEI ROKAN</h2>
        <a href="seirokantambah.php" class="btn-tambah">Tambah</a>
    </div>
